...
bool CEpgOptions::SaveEpgFile(LibISDB::EPGDatabase *pEPGDatabase)
{
	...
	if (m_fSaveEpgFile) {
		...
		if (pEPGDatabase->GetServiceCount() > 0) {
			...
			GetAppClass().AddLog(TEXT("EPG データを \"%") T_PRIS TEXT("\" に保存します..."), FilePath.c_str());
...
bool CEpgOptions::LoadLogoFile()
{
	if (m_fSaveLogoFile && !m_LogoFileName.empty()) {
		...
		if (FilePath.IsFileExists()) {
			App.AddLog(TEXT("ロゴデータを \"%") T_PRIS TEXT("\" から読み込みます..."), FilePath.c_str());
		...
		App.AddLog(TEXT("ロゴ設定を \"%") T_PRIS TEXT("\" から読み込みます..."), FilePath.c_str());
...
bool CEpgOptions::SaveLogoFile()
{
	if (m_fSaveLogoFile && !m_LogoFileName.empty()) {
		...
		if (!FilePath.IsFileExists() || LogoManager.IsLogoDataUpdated()) {
			App.AddLog(TEXT("ロゴデータを \"%") T_PRIS TEXT("\" に保存します..."), FilePath.c_str());
		...
		if (!FilePath.IsFileExists() || LogoManager.IsLogoIDMapUpdated()) {
			App.AddLog(TEXT("ロゴ設定を \"%") T_PRIS TEXT("\" に保存します..."), FilePath.c_str());
...
void CEpgOptions::CEpgFileLoader::LoadMain()
{
	if (m_pEpgDataStore != nullptr) {
		GetAppClass().AddLog(TEXT("EPG データを \"%") T_PRIS TEXT("\" から読み込みます..."), m_EpgDataPath.c_str());
	...
	if (m_pEdcbDataLoader != nullptr) {
		GetAppClass().AddLog(TEXT("EDCB の EPG データを \"%") T_PRIS TEXT("\" から読み込みます..."), m_EdcbDataFolder.c_str());
