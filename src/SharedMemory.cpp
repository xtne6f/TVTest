...
bool CSharedMemory::Create(LPCWSTR pszName, ULONGLONG Size, bool *pfExists)
{
	...
	m_hFileMapping = ::CreateFileMapping(
		...
		pszName);
	if (m_hFileMapping == nullptr) {
		TRACE(TEXT("CreateFileMapping \"%") T_PRIS TEXT("\" Error %x\n"), pszName, ::GetLastError());
		m_Lock.Close();
		return false;
	}

	TRACE(TEXT("File mapping created \"%") T_PRIS TEXT("\"\n"), pszName);
...
bool CSharedMemory::Open(LPCWSTR pszName, DWORD DesiredAccess, bool fInheritHandle)
{
	...
	if (m_hFileMapping == nullptr) {
		TRACE(TEXT("OpenFileMapping \"%") T_PRIS TEXT("\" Error %x\n"), pszName, ::GetLastError());
		m_Lock.Close();
		return false;
	}

	TRACE(TEXT("File mapping opened \"%") T_PRIS TEXT("\"\n"), pszName);
